...
extension APIServerResponse: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let valuesContainer = try? decoder.container(keyedBy: CodingKeys.self) {
            self.status = try valuesContainer.decodeIfPresent(APIServerResponseStatus.self, forKey: .errorMessage) == nil ? .success : .failure
            ...
            guard status == .success else {
                ...
                if let errorType = try? valuesContainer.decodeIfPresent(Int.self, forKey: .code) {
                    ...
                    if let message = try? valuesContainer.decodeIfPresent(String.self, forKey: .errorMessage) {
